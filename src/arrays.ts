...
export function tripleNumbers(numbers: number[]): number[] {
    let resultNumbers: number[] = [...numbers];
    resultNumbers = resultNumbers.map((num: number): number => num * 3);
...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    const withExclamation = messages.map((message: string): string =>
        message.endsWith("!") ? message.toUpperCase() : message
    );
    const removeQuestions = withExclamation.filter(
...
export function makeMath(addends: number[]): string {
    ...
    return sum.toString() + "=" + (addends.length ? result : "0");
}
...
export function injectPositive(values: number[]): number[] {
    const negativeNum = values.find((num) => num < 0);
    const negativeIndex =
        negativeNum !== undefined ? values.indexOf(negativeNum) : -1;

    if (negativeIndex === -1) {
        return [...values, values.reduce((a, b) => a + b, 0)];
    }

    const sum = values.slice(0, negativeIndex).reduce((a, b) => a + b, 0);
    return [
